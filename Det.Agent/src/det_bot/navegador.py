"""Abertura do Chrome nativo com perfil persistente por empresa.

Por que perfil persistente (``launch_persistent_context``) e não
``browser.new_context()``:

1. **Certificado A1 / TLS mútuo.** A autenticação por certificado no gov.br
   é um handshake TLS com autenticação de cliente. Quem escolhe e assina
   com a chave privada é o *navegador*, conversando com a Windows
   CryptoAPI (armazenamento de certificados do Windows) ou com o driver
   PKCS#11 do token/smartcard. Nenhuma API do Playwright participa desse
   handshake: por isso é obrigatório subir o Chrome instalado na máquina
   (``channel="chrome"``), com interface gráfica, e não o Chromium
   empacotado em modo headless.

2. **Reaproveitamento de sessão.** O ``user_data_dir`` guarda cookies,
   ``localStorage`` e decisões de certificado da sessão. Depois do primeiro
   login assistido, execuções seguintes tendem a cair direto no painel do
   DET sem novo diálogo de certificado, enquanto a sessão do gov.br durar.

3. **Isolamento entre identidades.** O gov.br mantém sessão única por
   navegador, então o diretório de perfil delimita *quem* está logado -- não
   *qual empresa* está sendo lida. Com um e-CNPJ por empresa, vale um perfil
   por empresa. Com o certificado do escritório e procuração a identidade é
   uma só: o perfil é compartilhado por todos os clientes, e quem separa um
   do outro é a troca de perfil dentro do DET
   (``govbr._selecionar_perfil_det``).
"""
...
from .settings import MODO_PFX, Config, Empresa
...
def _travar_perfil_em_uso(dir_perfil: Path) -> bool:
...
def _certificados_cliente(cfg: Config) -> list[dict[str, Any]] | None:
    ...
    Nesse modo o Playwright termina o TLS por conta própria e apresenta o
    .pfx à origem informada, dispensando o diálogo nativo do Windows. O
    certificado é **um só** -- o do escritório contábil --, e não varia por
    empresa: quem individualiza o cliente é a troca de perfil dentro do DET.
    """
    if cfg.modo_certificado != MODO_PFX:
        return None

    entrada: dict[str, Any] = {
        "origin": cfg.origem_certificado,
        "pfxPath": str(cfg.caminho_certificado()),
    }
...
def abrir_contexto(
    ...
    """Sobe o Chrome com o perfil persistente da empresa e devolve a aba ativa."""
    dir_perfil = (cfg.dir_perfis / empresa.dir_perfil).resolve()
    dir_perfil.mkdir(parents=True, exist_ok=True)

    if _travar_perfil_em_uso(dir_perfil):
        log.warning(
    ...
    parametros: dict[str, Any] = {
        ...
        "downloads_path": str(cfg.dir_saida / "downloads"),
        "locale": "pt-BR",
        "timezone_id": "America/Sao_Paulo",
        # Sem viewport fixo: a janela real (--start-maximized) é usada,
        # o que reduz layouts responsivos inesperados.
        "no_viewport": True,
    }
    ...
    if certificados:
        ...
        log.info("Certificado .pfx (%s) sera apresentado a %s",
                 Path(certificados[0]["pfxPath"]).name, cfg.origem_certificado)

    (cfg.dir_saida / "downloads").mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        raise ErroRobo(_diagnosticar_falha_navegador(exc, cfg, dir_perfil)) from exc
...
def _diagnosticar_falha_navegador(
    exc: ErroPlaywright, cfg: Config, dir_perfil: Path
) -> str:
    ...
    if "client_certificates" in texto or "pfx" in texto:
        return (
            ...
            "variavel de ambiente e se o arquivo e um PKCS#12 valido. Detalhe: "
            f"{exc}"
        )
...
def limpar_perfil(cfg: Config, empresa: Empresa, log: logging.Logger) -> None:
    """Descarta o perfil persistente da empresa (força novo login)."""
    dir_perfil = (cfg.dir_perfis / empresa.dir_perfil).resolve()
    if not dir_perfil.exists():
        log.info("Perfil '%s' ja estava ausente.", dir_perfil.name)
        return
    shutil.rmtree(dir_perfil, ignore_errors=True)
    log.info("Perfil '%s' removido.", dir_perfil.name)


def resumo_ambiente() -> dict[str, str]:
    """Metadados do ambiente, gravados no JSON para fins de auditoria."""
    return {

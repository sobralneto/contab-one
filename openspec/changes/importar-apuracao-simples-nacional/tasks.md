...
- [ ] 1.1 Acrescentar `TemAgente` (bool) ao `Produto` em
...
- [ ] 1.2 Acrescentar `Importacao = "importacao"` a `PaginaFerramenta.Todas`,
      com o comentário de qual componente do frontend ela corresponde
- [ ] 1.3 Expor `temAgente` em `GET /api/produtos` (`ProdutoDto`) e nos
      endpoints de admin de produtos (criar e atualizar), com default `true`
      quando ausente
- [ ] 1.4 Testes de API: catálogo devolve `temAgente`; página `importacao`
      aceita no cadastro; página desconhecida continua recusada

## 2. Modelo de dados e migration (API)

- [ ] 2.1 Entidades `ApuracaoSimples`, `ApuracaoSegregacao` e
...
- [ ] 2.2 Enums `TipoDocumentoPgdas` (`Extrato`, `Declaracao`) e
...
- [ ] 2.3 Mapear no `AppDbContext`: `HasQueryFilter` por `EscritorioId` nas
...
- [ ] 2.4 Migration única: criar as três tabelas, acrescentar
...
- [ ] 2.5 Rodar a migration no Postgres local e conferir: as três ferramentas
      no catálogo, `nfse`/`det` com `TemAgente = TRUE`, `pgdas` habilitado
      para os escritórios existentes

## 3. Slice `Features/Pgdas` (API)

- [ ] 3.1 `POST /api/pgdas/apuracoes` — grava o lote conferido de um cliente.
...
- [ ] 3.2 `GET /api/pgdas/apuracoes` — lista paginada, filtros por cliente e
      por intervalo de competência
- [ ] 3.3 `GET /api/pgdas/clientes/{clienteId}/dashboard` — o payload único
      da dashboard: apurações do intervalo, segregação, série mensal e
      identificação do cliente (nome e **CNPJ mascarado**)
- [ ] 3.4 `PUT /api/pgdas/apuracoes/{id}` (marca `EditadoManualmente`) e
      `DELETE /api/pgdas/apuracoes/{id}`
- [ ] 3.5 `GET /api/pgdas/resumo` — KPIs da visão geral: clientes com
...
- [ ] 3.6 Registrar o grupo `/api/pgdas` no `Program.cs`, com a mesma
      exigência de autenticação humana das demais rotas de painel
- [ ] 3.7 Testes de API: gravação de lote; 409 na competência repetida;
...
- [ ] 4.1 `POST /api/pgdas/clientes/identificar` — recebe CNPJ cru, deriva
...
- [ ] 4.2 `ClienteRequest` aceita `Cnpj` cru opcional; quando presente, a API
      deriva `CnpjHash` e `CnpjMascarado` e descarta o CNPJ. Cliente criado
      por esse caminho recebe `Origem = Importacao`
- [ ] 4.3 `GET /api/clientes/proximo-codigo` — próximo código numérico livre
      de quatro dígitos para o escritório em escopo
- [ ] 4.4 Testes de API: identificação por hash; identificação por máscara
...
- [ ] 5.1 Dependências: `pdfjs-dist`, `jspdf`, `html2canvas`; worker do
      `pdf.js` resolvido pelo Vite (`?url`), sem CDN
- [ ] 5.2 `src/features/pgdas/parser/extrairTexto.ts` — reconstrução das
      linhas por coordenada Y, tradução direta do original
- [ ] 5.3 `dividirApuracoes.ts` — corte por `Informações da Apuração` (extrato)
      e pelo cabeçalho `Declaratório` (declaração)
- [ ] 5.4 `parseApuracao.ts` — os dois leiautes, com todos os fallbacks de
      regex por campo, `semMovimento`, `pago` e o vencimento no dia 20 do mês
      seguinte
- [ ] 5.5 `segregacao.ts` e `historico.ts` — as quatro categorias fiscais e a
      série mensal da seção 2.2.1
- [ ] 5.6 `conferencia.ts` — soma dos oito tributos contra o DAS, tolerância
      R$ 0,05; a mesma regra que a API usa no resumo
- [ ] 5.7 Suíte Vitest do parser com fixtures de **texto já extraído** e
...
- [ ] 7.1 `views/pgdas/PgdasImportacaoView.vue` — carga e conferência com o
...
- [ ] 7.2 `views/pgdas/PgdasVisaoGeralView.vue` — apurações gravadas por
      cliente e competência, KPIs do resumo e as pendências em destaque
- [ ] 7.3 `views/pgdas/PgdasDashboardView.vue` — `iframe srcdoc` com o
      documento gerado; aguardar `load` antes de desenhar gráficos e antes de
      exportar
- [ ] 7.4 Exportação em PDF e em HTML a partir do `contentDocument` do
      iframe, preservando a quebra de página entre `dashPart1` e `dashPart2`
- [ ] 7.5 `api/endpoints/pgdas.ts` e os tipos correspondentes em
      `api/types.ts`
- [ ] 7.6 Testes Vitest de componente com MSW: conferência marcando a linha
...
- [ ] 8.1 `api/types.ts`: `'importacao'` em `PaginaFerramenta` e `temAgente`
      em `ProdutoDto`/`ProdutoAdminDto`
- [ ] 8.2 `PAGINA_META` em `layouts/AppLayout.vue`: rótulo "Importar
      extratos" para `importacao`
- [ ] 8.3 `router/index.ts`: `/f/:produto/importacao` (com
      `meta.pagina: 'importacao'`) e `/f/:produto/dashboard/:clienteId`
      (**sem** `meta.pagina` — rota de detalhe fora do menu)
- [ ] 8.4 `views/AgentesView.vue`: o seletor de nova chave passa a filtrar por
      `contratado && temAgente`
- [ ] 8.5 `views/admin/ProdutosView.vue`: checkbox "Tem agente" no formulário
      e a página `importacao` no checklist
- [ ] 8.6 `constants/explicacoesPagina.ts`: explicação das duas páginas novas
      da ferramenta
- [ ] 8.7 Testes de guard: rota de detalhe alcançável para ferramenta
      contratada e recusada para não contratada; `/f/pgdas/execucoes` (página
      não declarada) devolvendo à visão geral da ferramenta

## 9. Fechamento

- [ ] 9.1 E2E Playwright: entrar, abrir o `pgdas` pelo hub, importar um
...
- [ ] 9.2 Rodar as suítes: `dotnet test` e `npm test`; conferir que nenhuma
      fixture de PDF ou texto real de cliente entrou no repositório
- [ ] 9.3 Atualizar o `README.md` da raiz: a terceira ferramenta, e a
      distinção entre ferramenta com agente e sem agente no catálogo
- [ ] 9.4 Registrar no `design.md` as respostas das Open Questions que forem

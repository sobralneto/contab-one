...
**Reason**: A requisição descrevia exclusivamente os cards de ferramenta da
página inicial, que deixaram de existir — sem cards, não há card informativo de
ferramenta não contratada. O menu lateral, que passa a ser o único lugar de
navegação por ferramenta, já omite domínio sem ferramenta contratada para a
sessão de escritório (requisito "Domínio sem ferramenta contratada não aparece
para o escritório", inalterado), então não há onde encaixar o informativo sem
reintroduzir na navegação aquilo que ela deliberadamente esconde.

**Migration**: Nenhuma ação técnica — nada a migrar, nenhum endereço muda e o
guard de rota continua como está (ferramenta ativa não contratada segue
inalcançável para escritório e alcançável para o admin da plataforma). O efeito
de produto é que o escritório deixa de ter, na aplicação, qualquer vitrine das
ferramentas que ainda não contratou. Se essa vitrine voltar a ser desejada, ela
precisa de lugar próprio (uma página de catálogo, por exemplo), e não do painel
— que agora é sobre trabalho em aberto, não sobre o que se pode contratar.

# historico-simulador-simples-nacional Specification

## Purpose

Define a página de histórico do Simulador Simples Nacional
(`/simulador/historico`): a consulta das simulações que o escritório optou
por salvar, com ação de reabrir (recarrega os 4 campos gravados no
formulário e recalcula o resultado ali, nunca a partir de um valor
congelado) e de excluir definitivamente.

É a contraparte de leitura da persistência opcional descrita em
`simulacao-simples-nacional`: alcançada por um botão dentro do próprio
Simulador, sem entrada própria no menu do escritório, e isolada por
escritório como qualquer outro dado do painel — o histórico de um nunca
aparece para outro.

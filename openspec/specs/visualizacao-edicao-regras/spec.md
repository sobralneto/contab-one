...
O sistema DEVE (MUST) permitir que um PlatformAdmin visualize o conteúdo JSON completo de qualquer versão de regra publicada, sem precisar recriá-lo ou inspecionar o banco de dados.
...
O sistema DEVE (MUST) expor um endpoint que retorna uma regra individual incluindo seu conteúdo completo.
...
O sistema DEVE (MUST) permitir que o admin carregue o conteúdo de uma versão existente no editor de nova versão, para usar como ponto de partida.
...
O sistema DEVE (MUST) permitir que o admin copie o conteúdo JSON de uma versão existente para a área de transferência.

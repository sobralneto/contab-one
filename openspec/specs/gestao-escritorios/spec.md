...
Ao abrir o modal de edição de um escritório, o sistema DEVE (MUST) carregar e exibir o plano e o status atualmente vinculados ao escritório.
...
Na tabela de escritórios, o sistema DEVE (MUST) exibir o nome do status (ex: "Ativo", "Inativo") em vez do código numérico.
...
O sistema DEVE (MUST) persistir corretamente as alterações feitas no modal de edição de um escritório.
...
O sistema DEVE (MUST) persistir a alteração do CNPJ feita no modal de edição de um escritório.
...
O modal de cadastro de escritório DEVE (MUST) exibir os mesmos campos do modal de edição: Nome, CNPJ, Plano e Status.
...
O sistema DEVE (MUST) permitir cadastrar escritórios sem informar CNPJ, inclusive múltiplos escritórios consecutivos nessa condição.
